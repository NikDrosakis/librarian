...
function lochain(data,id){	
	...
	var cityval=ses("s_"+city),proval=ses("s_"+prov),stateval=ses("s_"+state),sel1='';        
...
function classifchain(data,id){	
	...
	var specval=ses("s_"+spec),subclassifval=ses("s_"+subclassif),classifval=ses("s_"+classif);        
...
if(my.grp==2 && (my.locationC >=3 || parseInt(coo('su_status')) >3)){ s.notify('warn',dic.REACH_MAX_LIMIT_LOCATION_CHANGE_CONTACT_ADMIN)}else {
...
coo('sid',exp[1]);
coo('grp',2);
xget({a:"dashu",b:coo("su_uid"),c:coo("su_status"),d:coo("su_uid")},o=>{"NO"!=o?"admiral"==local("prev_dash")?location.href=G.SITE_URL+"admin/admiral?mode=list":location.href=G.SITE_URL+"admin/user?mode=board&uid="+oldid:s.notify("error",dic.USERNAME_PASSWORD_NOT_CORRECT)});
}
},'json');
}
...
if (my.suspended && my.grp==2){
	...
	x('#goloader'+uid).show();
	// var session_group=coo('sid')==coo('affiliate') ? 2 : 1;
...
if(G.mode=='manager'){manager();}else{manage();}
	...
	coo(G.uname+'mode',page.id);
	coo.del(G.uname+'inter');
...
coo(G.uname+'_page',page);interloop();
}else if(G.uname=='mes'){		    
coo('page_num',page);
reset('wrapper1');
...
function searules(){							
		...
		}else if(my.grp==1 && my.hasOwnProperty('aff') && my.aff.sblock=='1' && !coo('agent_uid')){
...
if(uname=='mes'){	coo.del('mesuid');coo.del('mesid');	}
...
for(var r in reset_cookies){coo.del(reset_cookies[r]);location.href='/eoi/'+mode;}	
}
switch(uname){
case 'pa':
	...
	if(pages[i]!=id){		
	  x('#pb_'+pages[i]).hide();
		if(pages[i]!='search'){ses('pop_search',0);}
	}}	
//	if(my.grp==1 && G.is.potential_link=='0'){location.href='/u/'+my.name}					
	if(thisid.substr(thisid.length - 3)=="fav" && coo('sid')=="0"){
	...
	if(id=='search6'){ 
		...
		ses('s_mode', 'crit5');coo('s_mode', 'crit5');
		location.href='/home';		
	}			
	}	
	if(id=='search3'){
	ses('pop_search',3);ses('s_mode','upskill');location.href='/home/course?page=1';
	}	
	if(id=='search4'){ses('pop_search',4);ses('s_mode','crit4');location.href='/home/prop?page=1';
	}	
	if(id=='search4fav'){ses('pop_search',4);ses('s_mode','propfav');	location.href='/home/propfav?page=1';
	}
	if(id=='search2fav'){ses('pop_search',2);ses('s_mode','offerfav');location.href='/home/offerfav?page=1';
	}			
	if(id=='search2' || id=='search5'){
		...
		var link=s_mode=='hotels' || s_mode=='bars' ? s_mode : 'offer';
			...
			ses('s_mode',smode);
		if(id=='search5'){
			ses('s_mode2','business_sales');coo('s_mode2','business_sales');
		}else{
			ses.del('s_mode2');
			coo.del('s_mode2')
		}					
...
//}else{
//	x('#pb_'+id).hide();
//	if(id=='search'){ses('pop_search',0);}
//}
break;
case 'u':
var data=$(obj).data(); coo('anchor',data.link);
api.red.set("anchorhtml"+my.uid,$('#wrapper1').html());
location.href='/u/'+data.name+'/_'+data.mode+'/'+ data.link;
break;
case 'support':			
	var isaffiliate=my.agent > 0 ? 1:0;
	location.href= my.grp==1 && isaffiliate==1 && !coo('agent_uid') ? '/u/'+my.name+'/agent' : '/support/closed';
break;
case 'inter':			
coo('inter_page',1);ses.del('i_contact');
//x('#'+mode).attr('class','sortButtonInter');
//two weeks cancelled
if(mode=='icancelled'){ ses('i_status_param',1209600);}
else if(mode=='itoday'){ ses('i_status_param','today');}
else if(mode=='iconfirmed' && my.grp==1){ ses('i_status_param','today');}
else if(mode=='iupdaterecords'){ ses('i_status_param','updaterecords');}
...
function checksp(){
api.mo.getOne("sessions",{uid:coo('sid')},function(res){
var sp_check=res.sessions.includes(coo('sp'));
if (!sp_check){coo.delAll([]);location.href='/';}
})
}

function prologin(){
var wall= typeof (x('#us').val())!='undefined' ? x('#us').val() :'';
var inside= !!coo('prlgn') ? coo('prlgn') : '';
...
.on("click", "button[id^='jobBtn']", function () {
	...
	if (coo('job_tab') != id) {
		toggle_edit('boxJob' + coo('job_tab'), 'boxJob' + id);
...
if(my.have_dash){
var par={a: 'dash',b: my.dash.name,c: my.dash.pass,d:(coo('su_uid')!=null ? coo('su_uid') : my.uid),e: link}; //xget(par,function(data){
api.mo.getOne('sudos',{name:my.dash.name,pass:my.dash.pass,uid:my.uid},d=>{
		coo('dsh',coo('sp'));
		if (d!=undefined){
		if (G.CUR_DIR!='admin'){
			if (coo('ex-sid')!=null){
			location.href='/admin/user?mode=board&uid='+coo('ex-sid');                        
...
//if(!empty($dash)){coo('dsh',md5($_COOKIE['sp']));echo md5($_COOKIE['sp']);}else{echo 'NO';}
}
}
var keys={};
function printKeys(){
...
function readkey(key){if(G.uname=='offer'){return coo(key);}else{return ses("s_"+key);}}
function delkey(key){if(G.uname=='offer'){return coo.del(key);}else{return ses.del("s_"+key);}}
function savekey(key,val){if(G.uname=='offer'){return coo(key,val);}else{return ses("s_"+key,val);}}

function pcat(){ 		//app parameter	
//on load
var offerid=parseInt(G.id);
//το res δίνεται ses('pcat1''pcat2''pcat3')
...
.on("keyup",".affNotesText",function(){
	xget('?a=affnotes&b='+my.uid+'&c='+coo('affiliate')+'&d='+this.value);
})
.on("keyup","div[id^='comment2_']",function(){
var cid=this.id.replace('comment2_','');
var val=this.innerHTML;
...
function cardsopen(){
	if(!coo('cardsopen')){	
	coo('cardsopen',1); x('.subcard').showall();
	}else{
	coo.del('cardsopen');x('.subcard').hidall()
	}
}
function dash(){
	if (coo('dsh')!=coo('sp')){
	s.modal({title:dic.DASHBOARD_LOGIN,message:'<div class="bootContainer">'+dic.USER_NAME2+': <input class="bootInput" id="useradmin"></div><div class="bootContainer">'+dic.PASSWORD2+':<input type="password" class="bootInput" id="passadmin"></div>',foot:but("OK",function(){ //,pass:x('#passadmin').val()
		...
		console.log(d);	
		if(d!='NO'){coo('dsh',coo('sp'));location.href='/admin/dash';}else{s.notify('error',dic.USERNAME_PASSWORD_NOT_CORRECT)};})		
	})			
	})			
	}else{location.href='/admin/dash';}
}
...
xget({a: a},function(res){
	...
	// console.log('old list '+ses('notify'))
	// console.log('diff '+array_diff(JSON.parse(ses('notify')),notlist).length)
	var newnots=array_diff(JSON.parse(ses('notify')),notlist);
	if(newnots.length > 0){
	...
	ses('notify',JSON.stringify(notlist))
	x('#feed').html(feed)
},'json');
}
$(document).on("click","#proposal_received",function(){
coo('tab_settings','switchProfile04e');
location.href="/u/"+my.name+"/agent";
})			
...
if(G.mode=='favorite'){
	var selected= ses('fav_classif')==classifid ? 'selected=selected':'';
...
if(!uid0 || !uid || !table){
}else if(!!coo("SpeedemployerStreaming")){ 
	if(coo("SpeedemployerStreaming")!=link){
	s.confirm("New straming instance: "+link+". Other streaming instance is opened at room:"+coo("SpeedemployerStreaming")+" Do you want to switch rooms?",res=>{
		if(res){
			coo.del("SpeedemployerStreaming");
			coo('SpeedemployerStreaming',link);
...
var usergroup=  G.mode=='register' && G.uname=='home' ? 1 : (!coo('grp') ? pagegrp[G.mode] : coo('grp'))
var packs='';
...
var checkout={};
	...
	coo('checkout',JSON.stringify(checkout))
	...
	if(!!coo('checkout')){
   // console.log({a:'delete_invtemp',b:JSON.parse(coo('checkout'))['transaction']})
		xget({a:'delete_invtemp',b:JSON.parse(coo('checkout'))['transaction']},function(res){                
		coo.del('checkout');
		})
	}
	//$('#checkout').html('')
...
function upgrade(){
	...
	if([2,3].includes(coo('SPAUTH')) && parseInt(company_size) > 0){
		var size2mship={1:5,2:6,3:7};
...
.on("click","#confirmTerms",function(){
	...
	if (this.checked){				
			xget({a:'affiliate_terms',b:data.cid,c:1},data=>{
				if(G.uname=='eoi'){if(data!='OK'){coo('promise_terms',1);}}
			...
			xget({a:'affiliate_terms',b:data.cid,c:0},data=>{
				if(G.uname=='eoi'){coo.del('promise_terms');}
...
console.log(confirmationText(cvid,cid))	
	if (confirmTerms=='confirmTerms' && !coo('agent_uid')){
...
$.get('/ajax.php',{a:'check_auth',b:this.value},function(data){
	if(data.error_code==0){
		...
		coo('SPAFFAUTH',JSON.stringify(data))
		packs(1);
...
for(var i in contacookies){coo.del(contacookies[i]);}
}
function reset_filters_eoi(){
coo.del(['selectedPost','sort_eoi_subclass_'+G.mode,'sort_eoi_'+G.mode,
...
if(G.URL_PAGE=='eoi' && mode!=attr10){for(var r in reset_cookies){coo.del(reset_cookies[r]);}}
else if(this.id==6){coo.del('inter_page');ses('i_status',3);ses.del('i_status_param');}
else if(this.id==7){coo.del('inter_page');ses('i_status',4);ses.del('i_status_param');}
else if(this.id==8){coo.del('inter_page');ses('i_status',5);ses.del('i_status_param');}
//suspend prevent mes/chat
if(this.id==12 || this.id==13){
	coo.del('mesid');coo.del('mesuid');
if(my.suspended){
...
$.get('/ajax.php',{a:"setapp",b:id},res=>{			
	...
	ses('s_mode',G.searchfiles[G.app])
...
coo.del('authorid');coo.del('llset');x('#authorslist').hide();
x('#loginf').show();
x('#loginbox').show();
x('#loginf2').hide();
})
...
var divspec= ses('login_search')==1 ? 'sspec': 'sspec2';
if(ses('login_search')==1){
...
if(this.value==0){ ses.del('s_spec');}else{ses('s_spec',this.value);}
})
.on("click","#ssubclassif,#ssubclassif2",function()   {
// console.log('clicked '+this.id)
var divbutton=    ses('login_search')==1 ? '#submitCriteria2' : '#submitCriteria22';
...
specific_form2(subs,ses('s_spec'))
var subclasses=typeof subs !=='undefined' && subs.length > 0 ? JSON.stringify(subs,null,2) : '';
if(!!ses('s_subclassif') && x(divbutton).length>0){x(divbutton).attr("class",'but18');}
if (!!ses('s_subclassif') && ses('pop_search')==1){
...
var id=this.id.replace('authorid','');coo('authorid',id);authorlogin(id);
})
...
foot:"<a onclick='coo(&quot;LANG&quot;,&quot;en&quot;);location.href=&quot;register/"+grp+"&quot;'>English</a>"+ 
"<a onclick='coo(&quot;LANG&quot;,&quot;el&quot;);location.href =&quot;/register/"+grp+"&quot;'>Ελληνικά</a>",
}
};
...
if(!!coo('llset')){$("#loginwith-"+uid).attr('class','but136');}
...
function loginauthors(uid){
	...
	for(var i in res){	
	ali +='<div class="recentAd"><div class="photoRecentAdm"><span class="profNameRecAdm">'+res[i].fn+' '+res[i].ln+'</span><div class="profNameRecTit2con"><span class="profNameRecTit2">'+res[i].pt+'</span><span class="profNameRecTit2">'+G.authlevels[res[i].level]+'</span></div><input class="authorinput" type="password" id="author'+res[i].id+'" style="display:'+(coo('authorid')==res[i].id ? 'block':'none')+'" placeholder="password"><div class="login_butSm_box"><button type="button" id="authorid'+res[i].id+'" class="but96Sm"></button></div></div></div>';
	}
	ali +='</div>';
	x('#loginbox').hide();
...
var rc=!coo('rc') ? G.rcop[window.pathExt] : coo('rc');	
...
console.log(data)  
		if (data==='no_account'){
			...
			return false;
		} else if (data['auth']===1 && !coo('authorid')){										
		...
		if (result){
			if (data.authentication =='4'){
				coo('SPREGID',data.uid);coo('SPAUTH',2);coo('SPREGRP',data.grp);
				location.href="/register/employer";
			} else if (data.authentication =='3'){
				coo('SPREGID',data.uid);coo('SPAUTH',3);coo('SPREGRP',data.grp);
		...
		if(!!coo('llset') && coo('llset')!=data.uid){coo.del('logo')}		
...
var divspec= ses('login_search')==1 ? 'sspec': 'sspec2';
var divfspec= ses('login_search')==1 ? '#filter_specific': '#filter_specific2';
var form='';
...
home > service_pack
*/
// if ((URL_PAGE =='affiliate' && coo('affiliate_grp')==1) || G.mode=='requests'){
//     if(URL_PAGE!='proactivated'){
//          affloop()
//     }
// }
// if (G.CUR_DIR !='admin' && coo('affiliate_grp')==1){
//     affloop();
// }
...
.on("click",'#submit_mcli',function (){
	...
	coo.del(['job_status','newjobquote','sfuc','futoday','selectsub',G.apt+'_count','status_jobs','aff_order2']);
	manage();
})
...
.on("click","button[id^='order_'],button[id^='order2_']",function(){
	if(G.mode!='superboard' && G.uname!='user'){
		...
		var aff='aff_'+ele;
		var orderby=coo(aff)==q+' ASC' ? q+' DESC' : q+' ASC';				
		coo(aff,orderby);
...
function sm_go(){
		//run ajax
$('#step2').show();
	var prod=coo('sm_offerprod');
	var state=coo('sm_state');
	var province=coo('sm_province');
	var city=coo('sm_city');
	var address=coo('sm_address');
	if(address.length==0){
...
api.form("subclassifbyid_"+LOC,function(data){
	for(var i in arr){
		if(!subclassArr.includes(arr[i])){
			...
			sub +='<option value="'+arr[i]+'" '+(coo("selectsub")==arr[i] ?'selected="selected"' :'')+'>'+data[arr[i]].subclassif+'</option>';
		}
	}
...
var subclassArr=[],locArr=[],ajaxfile="/ajax.php",board='',page_num=!coo('page_num') ? 1 : coo('page_num'),
select_active=coo('select_active')!=null ? coo('select_active') : '',
aff_order=coo('aff_order')!=null && coo('aff_order') !='0' ? coo('aff_order') : '',
aff_search=coo('aff_search')!=null && coo('aff_search') !='0' ? coo('aff_search') : '',
aff_status= coo('aff_status')!=null && coo('aff_status') !='0' ? coo('aff_status') : '',   
agent=G.CUR_DIR =='admin' ? my.uid : coo('affiliate'),
...
//	getLocation2(d[i].uid1,d[i].city);
	if (G.mode =='list' && G.mode=='requests'){
		...
		board +='<tr id="box'+d[i].uid1+'">' +
			...
			'<td>'+d[i].activity+'</td>' +
			'<td>' +
			'<button onclick="coo(&quot;mesuid&quot;,'+d[i].cid+');location.href=&quot;/mes&quot;" class="but120" title="CURRENT_TOPIC"></button>'+
		...
		if (G.mobile){
			...
			if (G.CUR_DIR =='admin'){
				...
				board +='<tr id="fbox'+d[i].uid1+'"><td>'+d[i].uid1+'</td><td>'+d[i].code+'</td>' +
					...
					'<td><span style="float:left;width:100%;margin: 0 0 10px 0;">'+d[i].fname+(d[i].suspended ? '</span><span class="readMembershipLabel">'+dic.SUSPENDED+'</span>' : '')+'</td>' +
					'<td>'+G.client_service_alias[d[i].service]+'</td>' +
					'<td class="'+(coo('affiliate_grp')==1 ? 'classif'+d[i].id : (d[i].status ==4 ? 'classif'+d[i].id:''))+'" style="width:136px;"></td>' +
					'<td class="'+(coo('affiliate_grp')==1 ? 'locity'+d[i].uid1 : (d[i].status ==4 ? 'locity'+d[i].uid1:''))+ '" style="width:96px;"></td>' +
					'<td>'+(d[i].status==3 || d[i].status==4 ? '<input type="checkbox" id="servicequote_'+d[i].uid1+'" '+(d[i].servicequote==1 ? "checked" : "")+' value="'+d[i].servicequote+'">' : '')+'</td>' +
					'<td>'+(d[i].status==3 || d[i].status==4 ? '<input type="checkbox" id="minservicequote_'+d[i].uid1+'" '+(d[i].minservicequote==1 ? "checked" : "")+' value="'+d[i].minservicequote+'">' : '')+'</td>' +
					'<td>'+(d[i].takentime!=0 ? date('Y-m-d H:i',d[i].takentime) : '')+'</td>' +
			...
			if(d[i]['subclassif']!='-' && (coo('affiliate_grp')==1 || d[i].status ==4)){subclassArr.push(d[i]['subclassif']);}
			if(d[i]['city']!='-' && (coo('affiliate_grp')==1 || d[i].status ==4)){locArr.push(d[i]['city']);}
		}
	}                
$('.aflist').html(board)
...
for (var i in subclassArr){sub +='<option value="'+subclassArr[i]+'" '+(coo(div)==subclassArr[i] ? 'selected="selected"' : '')+'>'+G.subclassifbyid[subclassArr[i]].s+'</option>';}
x('#'+div).html(sub);    
}
...
for (var i in locArr3){sub +='<option value="'+locArr3[i]+'" '+(coo(div)==locArr3[i] ? 'selected="selected"' : '')+'>'+i+'</option>';}
x('#'+div).html(sub);          
}

function manage(){
console.log('running '+G.apt);
var html='',manage_page=!coo('manage_page') ? 1 : coo('manage_page');
...
* */
var manage_count= !coo('manage_count') || !!coo('mcli_q') || !!coo('job_status') || !!coo('newjobquote') || !!coo('sfuc') ? "" : parseInt(coo('manage_count'));
if(manage_count==""){
		xget({a: 'manage_counter'},d=>{	
	console.log("manage_count"+d)		
		if (d > 0){
			$('#manage_count').text(d);coo('manage_count',d);
		} else {
			$('#manage_count').html(0);
		}
	...
	//TEMP var params={datefrom: datefrom,dateto: dateto}
		...
		xget({a: 'manage_subs'},function(d){
			...
			//s.cors(CONF.apimongourl +"subclassifs",params,function(subs){
				...
				:0;												
					var selected=coo('selectsub')==d[i] ? 'selected="selected"' : '';
...
function manager(){
var html='',manager_page=!coo('manager_page') ? 1 : coo('manager_page');
...
/*     * GET client counter        * when        * */
var count= !coo('manager_count') ? "" : parseInt(coo('manager_count'));
	...
	var dateto= year+'-'+month+'-'+("0"+(dateObj.getDate()+1)).slice(-2);
		xget({a: 'manager_subs'},function(data){
			...
			//s.cors(CONF.apimongourl +"subclassifs",params,function(subs){
				...
				:0;												
					var selected=coo('selectsub')==data[i] ? 'selected="selected"' : '';
...
$('#loader').show();
var preventkey=coo('affiliate_grp')=="1" ? 'prevent_update_timer_int' : 'prevent_update_timer_ext';
...
.on("click","#affiliateSubmit",function(){
	...
	$.get('/ajax.php',{a:'transfer',b:auth},function(data){
		console.log(data)
			if (data =='1'){
				var agent=JSON.parse(coo('SPAFFAUTH'));
...
var ctermsd=!coo('ctermsd_'+cvid) ? 0 : coo('ctermsd_'+cvid);
var agnt= cvid!=0 && !!cvid ? coo('ctermsa_'+cvid) : coo('ctermsc_'+cid);
var agent=!agnt ? coo('ctermsc_0') : agnt;
...
if(!!id && id!='new'){ //reply
	...
	api.mo.fup("message",{_id:id},crit,d=>{
	  if (d.ok){
		  xget({a:"amail",b:coo('mesuid')},dat=>{console.log(dat);})
...
if (confirmTerms =='confirmTerms' && coo('agent_uid')==null){
var cid=exp[3];
...
console.log('mesopen');
	...
	//change style
	//var style= coo('mesid')==id ? 'rgb(253 253 253 / 12%)':'trasparent';
	if (uncounter==0){		
...
function mesmenuopen(cid,uid,topicid,id,callback){
	...
	d=>{
		...
		coo.del('mesid');coo('mesuid',uid);coo.del('mesmode');
...
if($(obj).hasClass('_confirmTerms') && !coo('agent_uid')){	
...
//coo.del('mesuid');
//coo.del('mesid');
var criteria={cid:cid,order:'created-'},html='<button onclick="mesmenuopen('+cid+','+uid+',null,'+id+')" class="mesmenu1">New Topic</button><button onclick="coo(&quot;mesuid&quot;,'+uid+');location.href=&quot;/mes&quot;" class="mesmenu1">Go to Message Page</button>';
...
console.log('mesloop');
	...
	var Gmode=!!mode ? mode : coo('mesmode'),
	meshtml='',page= !coo('page_num') ? 1 : coo('page_num'),count=Gmode=='all' ? parseInt(my.N["c_mes"]) : parseInt(my.N["c_mes_"+Gmode]),
	statuses={'deleted':0,'inbox':1,'favorite':2,'sent':3},
	mesuid=!!coo('mesuid') ? parseInt(coo('mesuid')): '';
//where=!!coo('mesuid')
	//? [{from:my.uid,to:parseInt(coo('mesuid'))},{from:parseInt(coo('mesuid')),to:my.uid}]
...
if(data.length>0){		
	...
	//	missingfn(data[i].topic,data[i].from,data[i].to);
		...
		var style= coo('mesid')==id ? 'blue':'trasparent';
...
a=coo('SPAUTH')==2 ? 'proactivation' : 'regSubmit',
uid= x('#id').val(),
...
if (captcha!=coo('SPPASSCHECK') && G.URL_PAGE!='admin' && G.uname!='home'){
...
if(local('name_exists')==1 && !['2','3'].includes(coo('SPAUTH'))){
...
xget(param,function(data){
	...
	if(price > 0){
		// checkoutroute();
		var checkout=JSON.parse(coo('checkout'));
		...
		coo('SPAUTH',3);coo('SPREGID',data.uid);coo('SPREGRP',data.grp);
	}else{
		s.modal(dic.REGISTRATION_SUCCESSFULLY);
		if(!!coo('SPAFFAUTH')){		
		var agent=JSON.parse(coo('SPAFFAUTH'))['uid'];
...
if(this.checked && !coo('SPAFFAUTH')){
x('#mship1').prop('checked',true);
// $('#mship1').click();
packs(1);
}else if(x(this).prop('checked') && !!coo('SPAFFAUTH')){
// $('#mship1').click();
packs(1);
//    $('#submitbox').show();
}else{
...
inter_page= !!coo('inter_page') && coo('inter_page') !=0 ? parseInt(coo('inter_page')) : 1,
iid=G.id !="" ? G.id : "",   
...
for (var t=1; t<=3;t++){
	...
	'<div class="iMin2" style="background-color:'+decodeURIComponent(G.iColorFlags[data[i].status])+'">' +
		...
		(!!coo('agent_uid') && data[i].agent==my.uid ? '<button class="mgt mgtin" onclick="coo(&quot;manage_page&quot;,1);coo(&quot;mcli_q&quot;,&quot;'+data[i].code+'&quot;);location.href=&quot;/home/manage&quot;">MGT</button>':'')+
...
.on("change","#i_contact",function(){ses('i_contact',this.value);coo('inter_page',1);interloop();})
.on("change","#i_post",function(){ses('i_post',this.value);coo('inter_page',1);interloop();})
...
ses('i_order',this.value);coo('inter_page',1);interloop();
})
.on("click","ul#select_type li",function(){
ses('i_type',this.value);coo('inter_page',1);interloop();
})
.on("click","ul#select_status li",function(){                
ses('i_status',this.value);coo('inter_page',1);interloop();
})
.on("click","ul#i_contact li",function(){
ses('i_contact',this.value);coo('inter_page',1);interloop();
})
.on("click","ul#i_post li",function(){
ses('i_post',this.value);coo('inter_page',1);interloop();
})
.on("change","#selInter",function(){
ses('i_order',this.value);coo('inter_page',1);                
interloop();
})
...
coo('i_status',this.value);coo('inter_page',1);               
...
var sort_contact_cookie=coo(sort_contact);
//sort 2 get cookie
var sort_contact_sub='sort_contact'+smode+'_sub';
var sort_contact_sub_cookie= coo(sort_contact_sub)
...
if(this.value==0){coo.del(cooki);}else {
coo(cooki,this.value);
}
...
.on("change",'#selectedPost',function (){if (coo('selectedPost',this.value));location.href='/eoi/'+G.mode;})
...
.on("change",'#sort_eoi_subclass_'+G.mode,function (){
	 if (this.value==0){coo.del(this.id);} else {coo(this.id,this.value);}
	 location.href='/eoi/'+G.mode;
})	
...
var cline=!lin ? coo("chatline"+G.ui) : lin;
...
.on("click","button[id^='closechat']",function(){var id=this.id.replace('closechat','');x("#chatbox"+id).remove();chat.coo(id,0,this);})
.on("click","button[id^='chat_']",function(){var line=explode('_',this.id);chat.coo(line[1],1,this)})
...
.on("click","button[id^='bidsubmit']",function(){
	...
	var expiredbid= coo('proptype')==2 ? my.propbitleaseE==0 || my.propbitleaseE <= time() : my.propbitsaleE==0 || my.propbitsaleE <= time();
	var service=coo('proptype')==2 ? 15 : 16;
		...
		if(res=="EXIST"){
			...
			$('#send'+G.pt+'eoi_'+exp[1]+'_'+exp[2]).text('eoi sent');
				...
				location.href='/prop';
					...
					if(res!='NO' && res!='exist'){	chat.coo(cid,1,thi);thi.remove();}else{s.notice('log',res)}
				}else{
...
coo.del('offerid');coo.del('offeruid');coo.del('offerprod');coo.del('offergrp');coo.del('pcat1');coo.del('pcat2');coo.del('pcat3');
coo.del('offerinter');coo.del('offerflag');coo.del('offerplace');coo.del('offerprod');coo.del('offertag');coo.del('offereoiorder');
}
/*
...
api.mo.fup("sessions",{uid:  parseInt(coo('sid'))},{$pull:{sessions:sp}},d=>{console.log(res);$('#list'+sp).hide();})
})
...
$.get('/ajax.php',{a: 'appendsubcont',b: b,c: G.uname},function(data){
   if (sizeobj(data.subs) > 0){
		var subs=data.subs;
		for (var j in subs){
			sub +='<option value="'+subs[j]+'" '+(coo(id)==subs[j] ? "selected=selected" : "")+'>'+j+'</option>';
		}
	...
	if (my.grp==1 && sizeobj(data.bg) > 0){
		var bg=data.bg;
		for (var j in bg){
			bsh +='<option value="'+bg[j]+'" '+(coo('sel_buscat')==bg[j] ? "selected=selected" : "")+'>'+j+'</option>';
		}
...
if (data !='NO'){
 ...
 $.unique(prkey.sort());
 for (var j in prkey){
	 sub +='<option value="'+profs[prkey[j]]+'" '+(coo(cookieselected)==profs[prkey[j]] ? "selected=selected" : "")+'>'+prkey[j]+'</option>';
 }
...
if(my.grp==2){
 ...
 for (var j in skey){
	var selectedsk= coo('skills')==j ? "selected=selected" : "";
...
//FIXING TITLES
 ...
 $.unique(key.sort());
 for (var j in key){
		  pst +='<option value="'+keys[key[j]]+'" '+(coo('selectedPost')==keys[key[j]] ? "selected=selected" : "")+'>'+key[j]+'</option>';
	  }
...
coo('my.uid',data['uid'],domain);
coo('SPNAME',data['name'],domain);
coo('my.grp',data['grp'],domain);
coo('rc',domain,domain);
//for (i in G.constants){
//coo(i,data[G.constants[i]]);
//}
if (data['status'] > 1){
	coo('SPSUGROUP',data['status'],domain);
}
...
function potloop(id){
	...
	page= !!coo('potential_page') && coo('potential_page')!=0 ? parseInt(coo('potential_page')) : 1,
	order= !!coo('potential_order') && coo('potential_order') !=0 ? parseInt(coo('potential_order')) : 1;
	...
	var cookie_province=coo('potential_province');
...
method=res.tobesaved.includes(cvs[i].id)
	? (res.pot_save_method=="auto"
		...
		: (!coo('prevent_reload'+cvs[i].id) ? '<span id="loader'+cvs[i].id+'" style="display:none" class="loaderSm"></span>'+
...
coo('potential_unblocked2',1);location.href='/potential?id=' +id;
}	 
}
//pagination event
$(document)
...
.on("click","button[id^='agentrenew']",function(){
	...
	if(totalprice > 0){
	var checkout=!coo('checkout') ? {} : JSON.parse(coo('checkout'));
...
var service=11;
var redirect='/u/'+my.name+'/offer';
coo('checkout',JSON.stringify({orderDesc:desc,orderAmount:totalprice,uid:my.uid,service:service}))
var status= totalprice > 0 ? 3 : 1;
var payMethod= $("input[name='payMethod']").val();
var payment_type=  typeof(payMethod) !='undefined' ? payMethod : '';
...
api.ma.my('get',my,["user","mship","agentdata","pack","packdetails","caff"]);
	if (data !='nobank' && data !='error'){		
		if (agentype =='3'){
			coo('affiliate',my.uid);
			coo('affiliate_grp',my.affiliate);
		}
...
//TOPIC
//new message
$(document).on("change","#strSortBy",()=>{coo('support',this.value);$('.settingBoxMessages').html('');tktloop();})
...
function tktloop(){
var support='',page=!coo('page') ? 1: parseInt(coo('page')),count=parseInt(my.N["c_support_"+G.mode]),tktstatus={"closed": 0,"sent": 1},
...
//var pms={limit:10,page_num: !coo('spage') ? 1: coo('spage')};
...
function seafilter(obj){ 
	...
	coo('s_range_subclassif',JSON.stringify(rangesub));
...
//	}	
	//TEMP search_updated();	
	//TEMP saveSearchObj();
var page=!coo('spage_page') ? 1:parseInt(coo('spage_page'));
searun('search',pms)
.then(
...
function searun(a,pms){  //runs a) sealoop b) counter c) 	
	...
	pms.a=a,pms.limit=10,pms.page_num=!coo('spage_page') ? 1: parseInt(coo('spage_page'));
...
//	seaexe(pms)			
	...
	coo('s_mode','matching');ses('s_mode','matching');	
	x('#crit').hide();	
	...
	readMatching('#radiopostform2');coo('s_mode','matching2');
	...
	coo('s_mode','euser');ses('s_mode','euser');
	...
	if($('.crit4agent').css('display')=='none'){
		...
		coo('s_mode','crit4');ses('s_mode','crit4');
	}
	...
	coo('s_mode','crit');ses('s_mode','crit');
}	
...
function search_matching(selected){
	...
	if(!!ses('s_range_subclassif')){
		coo('s_range_subclassif',ses('s_range_subclassif'));
	}	
}
